 ...
 * stream,
 * or getHandleMaps() to get a list of all handle->content maps generated during
 * parsing.
 * The objects are generally instances that implement the interface "content";
 * see the
 ...
 * The
 * primary output consists of three separate stages. The first stage is a
 * textual
 * description of every piece of content in the stream, in the order it was
 * written.
 * There is generally a one-to-one mapping between ObjectOutputStream.writeXXX()
 * calls and
 ...
 * available to govern this stage, including -filter, -showarrays, -noclasses,
 * and
 * -fixnames. <br/>
 * <br/>
 * The third stage is a dump of every instance embedded inside the stream,
 * including
 * textual descriptions of field values. This is useful for casual viewing of
 * class data.
 ...
 * option.
 * Additionally, a manifest describing the size of each individual block can be
 * generated
...
public class jdeserialize {
    ...
    /**
     * <p>
     * Retrieves the list of content objects that were written to the stream. Each
     * item
     * generally corresponds to an invocation of an ObjectOutputStream writeXXX()
     * method.
     ...
     *
     * <p>
     * Entries in the list may be null, because it's perfectly legitimate to write a
     * null
     ...
     * multiple
     * logical maps: when a reset happens (indicated by TC_RESET), the current map
     * is
     * cleared.
     * </p>
     *
    ...
    public static String resolveJavaType(fieldtype type, String classname, boolean convertSlashes, boolean fixname)
            throws IOException {
    ...
    /**
     * "Fix" the given name by transforming illegal characters, such that the end
     * result
     ...
     * @return the unmodified string if it is legal, otherwise a legal-identifier
     *         version
     */
    ...
    public static void dump_ClassDesc(int indentlevel, classdesc cd, PrintStream ps, boolean fixname)
            throws IOException {
        ...
        if (cd.classtype == classdesctype.NORMALCLASS) {
            ...
            ps.print("class " + (classname.charAt(0) == '[' ? resolveJavaType(fieldtype.ARRAY, cd.name, false, fixname)
                    : classname));
    ...
    /**
     * Read the content of a thrown exception object. According to the spec, this
     * must be
     * an object of type Throwable. Although the Sun JDK always appears to provide
     * enough
     * information about the hierarchy to reach all the way back to
     * java.lang.Throwable,
     * it's unclear whether this is actually a requirement. From my reading, it's
     * possible that some other ObjectOutputStream implementations may leave some
     * gaps in
     * the hierarchy, forcing this app to hit the classloader. To avoid this, we
     * merely
     * ensure that the written object is indeed an instance; ensuring that the
     * object is
    ...
    /**
     * <p>
     * Read the next object corresponding to the spec grammar rule "content", and
     * return
     ...
     * one case where this isn't true is when an exception is embedded inside
     * another
     * object. When this is encountered, only the serialized exception object is
     * returned; it's up to the caller to backtrack in order to gather any data from
     * the
     * object that was being serialized when the exception was thrown.
     * </p>
     *
     * @param tc        the last byte read from the stream; it must be one of the
     *                  TC_* values
     ...
     *
     * <p>
     * If shouldConnect is true, then jdeserialize will attempt to identify member
     * classes
     * by their names according to the details laid out in the Inner Classes
     * Specification. If it finds one, it will set the classdesc's flag indicating
     * that
        ...
        try {
            ...
            if (magic != ObjectStreamConstants.STREAM_MAGIC) {
                throw new ValidityException(
                        "file magic mismatch!  expected " + ObjectStreamConstants.STREAM_MAGIC + ", got " + magic);
            }
            short streamversion = dis.readShort();
            if (streamversion != ObjectStreamConstants.STREAM_VERSION) {
                throw new ValidityException("file version mismatch!  expected " + ObjectStreamConstants.STREAM_VERSION
                        + ", got " + streamversion);
            }
            reset();
     ...
     * retain an instance of P, and therefore P's class description doesn't need to
     * be
    ...
    public void connectMemberClasses() throws IOException {
                ...
                if (!clmat.matches()) {
                    throw new ValidityException(
                            "inner class enclosing-class reference field exists, but class name doesn't match expected pattern: class "
                                    + cd.name + " field " + f.name);
                }
                String outer = clmat.group(1), inner = clmat.group(2);
                classdesc outercd = classes.get(outer);
                if (outercd == null) {
                    throw new ValidityException(
                            "couldn't connect inner classes: outer class not found for field name " + f.name);
                }
                if (!outercd.name.equals(f.getJavaType())) {
                    throw new ValidityException("outer class field type doesn't match field type name: "
                            + f.classname.value + " outer class name " + outercd.name);
                }
        ...
        for (classdesc ncd : newnames.keySet()) {
            ...
            if (classnames.contains(newname)) {
                throw new ValidityException(
                        "can't rename class from " + ncd.name + " to " + newname + " -- class already exists!");
            }
            for (classdesc cd : classes.values()) {
                if (cd.classtype == classdesctype.PROXYCLASS) {
                    continue;
            ...
            if (classnames.remove(ncd.name) == false) {
                throw new ValidityException(
                        "tried to remove " + ncd.name + " from classnames cache, but couldn't find it!");
            }
            ncd.name = newname;
     ...
     * @param convertSlashes true iff slashes should be replaced with periods (true
     *                       for
     *                       "real" field-descriptor format; false for names in
     *                       classdesc)
